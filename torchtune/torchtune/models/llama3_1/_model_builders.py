...
from torchtune.models.llama3_1._component_builders import llama3_1, lora_llama3_1, llama3_1_segment, llama3_1_seg_attnbias, llama3_1_attnbias

from torchtune.modules import TransformerDecoder, TransformerDecoderSeg
...
def llama3_1_8b(max_seq_len: int = 131072) -> TransformerDecoder:
    """
...
def llama3_1_8b_segment(max_seq_len: int = 131072) -> TransformerDecoderSeg:
    return llama3_1_segment(
        segment_size=4,
        vocab_size=128_256,
        num_layers=32,
        num_heads=32,
...
def llama3_1_8b_seg_attnbias() -> TransformerDecoderSeg:
    """ISE (segment embeddings) + learnable role-key attention bias.

    Extends llama3_1_8b_segment() with a role_key_bias parameter that
    adds a learnable scalar to attention logits per role (sys/user/data/asst).
    This counteracts RoPE recency bias, allowing the model to learn to
    attend more strongly to high-priority instructions regardless of position.
    """
    return llama3_1_seg_attnbias(
        segment_size=4,
        vocab_size=128_256,
        num_layers=32,
        num_heads=32,
...
def llama3_1_8b_attnbias() -> TransformerDecoderSeg:
    """AttnBias-only baseline: learnable role-key attention bias, no segment embeddings.

    Trained from the base model to isolate the contribution of attention-level
    role routing vs. ISE's representation-level segment embeddings.
    """
    return llama3_1_attnbias(
        vocab_size=128_256,
        num_layers=32,
        num_heads=32,
        num_kv_heads=8,
