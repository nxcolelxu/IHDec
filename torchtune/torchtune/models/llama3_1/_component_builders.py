...
def llama3_1_segment(
    ...
    hidden_dim = intermediate_dim if intermediate_dim else scale_hidden_dim_for_mlp(embed_dim)
    mlp = llama3_segment_mlp(dim=embed_dim, hidden_dim=hidden_dim)
    layer = TransformerDecoderLayer(
        attn=self_attn,
        mlp=mlp,
        sa_norm=RMSNorm(dim=embed_dim, eps=norm_eps),
    ...
    tok_embeddings = nn.Embedding(vocab_size, embed_dim)
    seg_embeddings = nn.Embedding(segment_size, embed_dim)
    nn.init.normal_(seg_embeddings.weight, mean=0, std=0.01)
    output_proj = nn.Linear(embed_dim, vocab_size, bias=False)
    return TransformerDecoderSeg(
        tok_embeddings=tok_embeddings,
        layer=layer,
        num_layers=num_layers,
        max_seq_len=max_seq_len,
...
def llama3_1_attnbias(
    ...
    """AttnBias-only: learnable role-key attention bias, NO segment embeddings.

    Compared against ISE (llama3_1_segment) to isolate the contribution of
    role-aware attention routing vs. role-aware token representations.
    """
    head_dim = embed_dim // num_heads
    num_kv_heads = num_kv_heads if num_kv_heads else num_heads
    rope = Llama3ScaledRoPE(dim=head_dim, max_seq_len=max_seq_len, base=rope_base)
...
def llama3_1_seg_attnbias(
    ...
    """ISE + learnable role-key attention bias (H-RoPE-style).

    Identical to llama3_1_segment() but enables role_key_bias=True in
    TransformerDecoderSeg, adding a learnable per-role scalar that is
    added to attention logits for every key token of that role.

    This directly counteracts RoPE recency bias: the model can learn to
    boost attention to system-prompt tokens regardless of their position.
    """
    head_dim = embed_dim // num_heads
    num_kv_heads = num_kv_heads if num_kv_heads else num_heads
    rope = Llama3ScaledRoPE(dim=head_dim, max_seq_len=max_seq_len, base=rope_base)
...
def llama3_segment_mlp(dim: int, hidden_dim: int) -> FeedForward:
    """
    Build the MLP layer associated with the Llama model.
    """
    gate_proj = nn.Linear(dim, hidden_dim, bias=False)
